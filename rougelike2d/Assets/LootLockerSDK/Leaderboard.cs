    ...
    public IEnumerator FetechPersonalScoreRoutine()
    {
        ...
        LootLockerSDKManager.GetMemberRank(leaderboardID, PlayerManager.instance.GetPlayerID(), (response) =>
        {
            if (response.success)
            {
                ...
                string tempPersonalName = "Your Rank\n <br>";
                string tempPersonalScore = "Highscore\n <br>";

                tempPersonalName = tempPersonalName + response.rank.ToString() + ". " + response.player.name;
                tempPersonalScore = tempPersonalScore + response.score.ToString();

                personalNameTxt.text = tempPersonalName.ToString();
                personalScoreTxt.text = tempPersonalScore.ToString();
